...
class ViewController: UIViewController {
    ...
    lazy var game = Concentration(numberOfParsOfCards: (cardButtons.count + 1) / 2)
    var flipcount = 0 {
        didSet {
    ...
    @IBAction func touchCard(_ sender: UIButton) {
        ...
        if let cardNumber = cardButtons.index(of: sender) {
            ...
            updateViewFromModel(indexPhrase: cardNumber)
        } else {
            print("chosen card was not in cardButtons")
        }
    }
    
    func updateViewFromModel(indexPhrase: Int) {
        for index in cardButtons.indices {
            let button = cardButtons[index]
            let card = game.cards[index]
            if card.isFaceUp {
                
                // Make custom buttom with two lines
                let rangeOfString = (phraseChoices[indexPhrase]) as NSString
                let newlineRange: NSRange = rangeOfString.range(of: "\n")
                ...
                if(newlineRange.location != NSNotFound) {
                    substring1 = rangeOfString.substring(to: newlineRange.location) as NSString
                    substring2 = rangeOfString.substring(from: newlineRange.location) as NSString
                }
                ...
                //second substring
                let font1 = UIFont(name: "Arial", size: 9.0)
    ...
    func phrase(for card: Card) -> String {
        <#function body#>
    }
}
